...
X_train_data = loadMNISTImages('../../common/data/train-images-idx3-ubyte');
X_test_data = loadMNISTImages('../../common/data/t10k-images-idx3-ubyte');
X_train_data = X_train_data(:,1:1000);
rankX = rank(X_train_data)
%[X, ~] = licols(X(:,1:60000));

%% Parameters
[D, N] = size(X_train_data)
K = 1000 % hidden units
lambda = 0;
%% parameter initilization ------------------------------------------------
%one_letter_of_each = X(:,[1:6,8,14,16,18]);
%t_initial = [one_letter_of_each, datasample(X', K-10, 'Replace', false)' ];
t_initial = datasample(X_train_data', K, 'Replace', false)';
%% Learn
%beta = 1e-7
beta = 1;
%% K data
...
while rank(Ksub) ~= K
    ...
    Kdata = produce_kernel_matrix(X_train_data, t_initial, beta);
    [Ksub, ~] = licols(Kdata);
    RANK = rank(Ksub)
end
%% Kdagger
...
%%
error_training_initial_model = compute_Hf_sq_error(X_train_data,X_train_data, mdl_initial, lambda)
%error_test_initial_model = compute_Hf_sq_error(X_test_data,X_test_data, mdl_initial, lambda)

error_training_final_model = compute_Hf_sq_error(X_train_data,X_train_data, mdl_final, lambda)
%error_test_final_model = compute_Hf_sq_error(X_test_data,X_test_data, mdl_final, lambda)

%% Visualize data TRAIN
originalTrainingX = X_train_data(:,1:30);
...
title('reconstructionTrainingX LinearAlgebra')
%normalized
% figure
