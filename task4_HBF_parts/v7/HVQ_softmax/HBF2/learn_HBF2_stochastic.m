function [ mdl_new ] = learn_HBF2_stochastic(X,y, mdl, mu_c,mu_t1,mu_t2, lambda, visualize,iterations)
...
[K2, L] = size(mdl.c);
[~, Dd, Np] = size(mdl.t1);
current_error = compute_Hf_softmax(X,y, mdl, lambda);
errors = cell(1,1);
...
mdl_new = HBF2(mdl.c, mdl.t1, mdl.t2, mdl.beta);
for i=1:iterations
    ...
    y_i = y(i_rand);
    %% get new parameters
    [ h_i, z_l1, z_l2, a_l2, a_l3 ] = h(x_i,mdl_new.c,mdl_new.t1,mdl_new.t2,mdl_new.beta );
    [c_new, dJ_dc] = update_c_gradient(mdl_new.c, h_i,y_i, a_l3, mu_c, lambda);
    [t1_new, dJ_dt1] = update_t1_gradient(mdl_new.t1, h_i,x_i,y_i, z_l1,z_l2,a_l2, mdl_new.c,mdl_new.t2,mdl_new.beta, mu_t1, lambda);
    [t2_new, dJ_dt2] = update_t2_gradient(mdl_new.t2, h_i,y_i, z_l2,a_l2, mdl_new.c,mdl_new.beta, mu_t2,lambda);
    ...
    change_t1_wrt_iteration = get_dt1_diter(mdl_new.t1, t1_new);
    ...
    change_t2_wrt_iteration = get_dt2_diter(mdl_new.t2, t2_new );
    ...
    current_error = compute_Hf_softmax(X,y, mdl_new, lambda);
