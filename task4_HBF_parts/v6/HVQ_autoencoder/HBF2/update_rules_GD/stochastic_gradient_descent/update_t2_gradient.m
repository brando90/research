function [ t2_new, dJ_dt2 ] = update_t2_gradient(t2, h,y, z_l2,a_l2,c,sig, mu_t2, lambda)
...
for l=1:L
    c_l = c(:, l);
    df_dt2_l = compute_df_dt2_loops(t2, z_l2,a_l2, c_l, sig); % (K1 x K2)
    df_dt2_M(:,:, l) = df_dt2_l;
end
dJ_df_l = ((1:L) == y) -  prob_y_x(h)'; % (1 x L)
