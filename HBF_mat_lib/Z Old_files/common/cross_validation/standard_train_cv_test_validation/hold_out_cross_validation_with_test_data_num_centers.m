function [ best_mdl, error_test_best_mdl] = hold_out_cross_validation_with_test_data_num_centers(data_for_cross_validation, centers, params4mdl_iter, visualize)
%   Input:
%   Output:
...
iterations_centers = length(centers);
list_train_errors = zeros(1,iterations_centers);
list_test_errors = zeros(1,iterations_centers);
...
for k=1:iterations_centers
    ...
    if visualize
        list_train_errors(k) = compute_Hf_sq_error( X_train,y_train, current_trained_mdl, lambda );
        list_test_errors(k) = compute_Hf_sq_error( X_test,y_test, current_trained_mdl, lambda );
